...
RelationshipTracker.prototype = {
  ...
  // creates a relationship + childhub for a single parent (no partner)
  createOrGetSingleParentChildhub: function (parentID) {
    // use a special key for single-parent relationships
    var singleKey = '__single__';
    if (this.relationships.hasOwnProperty(parentID) && this.relationships[parentID].hasOwnProperty(singleKey)) {
      var relID   = this.relationships[parentID][singleKey];
      var chhubID = this.relChildHubs[relID];
    } else {
      if (this.relationships[parentID] === undefined) {
        this.relationships[parentID] = {};
      }
      ...
      this.newG.addEdge( relID,    chhubID, this.defaultEdgeWeight );
      this.newG.addEdge( parentID, relID,   this.defaultEdgeWeight );

      this.relationships[parentID][singleKey] = relID;
      this.relChildHubs[relID] = chhubID;
    }
