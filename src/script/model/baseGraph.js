...
BaseGraph.prototype = {
    ...
    for (var v = 0; v < this.v.length; v++) {
      ...
      if (this.isPerson(v)) {
        ...
        if (inEdges.length < 1 || inEdges.length > 2) {
          throw 'Assertion failed: relationships should have one or two associated persons (failed for ' + this.getVertexDescription(v) + ')';
        }
  ...
  getParents: function(v) {
    ...
    // skips through relationship and child nodes and returns an array of real parents (1 or 2). If none found returns []
    ...
    if (inEdges.length < 1 || inEdges.length > 2) {
      throw 'Assertion failed: one or two parents expected';
    }

    var result = [this.upTheChainUntilNonVirtual(inEdges[0])];
    if (inEdges.length > 1) {
      result.push(this.upTheChainUntilNonVirtual(inEdges[1]));
    }
    return result;
  },

  getPathToParents: function(v) {
    // returns an array with one or two elements: path to parent(s) (excluding v):
    // [ [virtual_node_11, ..., virtual_node_1n, parent1], [virtual_node_21, ..., virtual_node_2n, parent2] ]
  ...
  getRelationshipNode: function(v1, v2) {
    ...
    for (var r = 0; r < relationships.length; ++r) {
      ...
      if (partners[0] === v2 || (partners.length > 1 && partners[1] === v2)) {
        return relationship;
      }
    }
    return null;
