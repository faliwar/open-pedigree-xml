...
DynamicPositionedGraph.prototype = {
  ...
  isSingleParentRelationship: function( v ) {
  ...
  getPosition: function( v ) {
    ...
    // Force perfectly straight vertical line for single-parent families
    if (this.DG.GG.isRelationship(v)) {
      var parents = this.DG.GG.getParents(v);
      if (parents.length === 1) {
        x = this.DG.positions[parents[0]];
      }
    } else if (this.DG.GG.isChildhub(v)) {
      var rels = this.DG.GG.getInEdges(v);
      if (rels.length === 1 && this.DG.GG.isRelationship(rels[0])) {
        var parents = this.DG.GG.getParents(rels[0]);
        if (parents.length === 1) {
          x = this.DG.positions[parents[0]];
        }
      }
    ...
    if (this.isRelationship(v)) {
      ...
      var level2   = (partners.length > 1 && this.DG.vertLevel.outEdgeVerticalLevel[partners[1]]) ?
        (this.DG.vertLevel.outEdgeVerticalLevel[partners[1]].hasOwnProperty(v) ? this.DG.vertLevel.outEdgeVerticalLevel[partners[1]][v].verticalLevel : 0) : level1;
      ...
      var attach2  = (partners.length > 1 && this.DG.vertLevel.outEdgeVerticalLevel[partners[1]]) ?
        (this.DG.vertLevel.outEdgeVerticalLevel[partners[1]].hasOwnProperty(v) ? this.DG.vertLevel.outEdgeVerticalLevel[partners[1]][v].attachlevel : 0) : attach1;
  ...
  addNewChildWithoutPartner: function( personId, childProperties, numTwins ) {
    this._debugPrintAll('before');
    var timer = new Timer();

    if (!this.DG.GG.isPerson(personId)) {
      throw 'Assertion failed: adding child without partner to a non-person node';
    }
    ...
    // find the best order to use for this new vertex
    var insertOrder = this._findBestInsertPosition( insertRank, personId );

    // create the relationship node connected only to this single parent
    var newRelationshipId = this._insertVertex(BaseGraph.TYPE.RELATIONSHIP, {}, 1.0, personId, null, insertRank, insertOrder);
    ...
    // fix common layout mistakes and update vertical positioning of all edges
    this._heuristics.improvePositioning(ranksBefore, rankYBefore);

    // update ancestors
    this.updateAncestors();

    timer.printSinceLast('=== NewChildWithoutPartner runtime: ');
    this._debugPrintAll('after');
  ...
  fromImport: function (importString, importType, importOptions) {
    ...
    } else if (importType == 'invitae') {
      var baseGraph = PedigreeImport.initFromInvitae(importString);
      if (!this._recreateUsingBaseGraph(baseGraph)) {
        return null;
      }  // no changes
    }
    ...
    for (var i = 0; i <= maxOldID; i++) {
      ...
      if (this.DG.GG.type[i] == BaseGraph.TYPE.RELATIONSHIP || this.DG.GG.type[i] == BaseGraph.TYPE.PERSON) {
        ...
        if (this.DG.GG.type[i] == BaseGraph.TYPE.RELATIONSHIP) {
          ...
          if (inEdges[0] > this.DG.GG.maxRealVertexId || (inEdges.length > 1 && inEdges[1] > this.DG.GG.maxRealVertexId)) {
            result[i] = true;
            continue;
          }
          ...
          if (vertLevelsBefore.outEdgeVerticalLevel[parents[0]] !== undefined &&    // vertical levels may be outdated if multiple nodes were created in one batch
                        (parents.length < 2 || vertLevelsBefore.outEdgeVerticalLevel[parents[1]] !== undefined)) {
            if (vertLevelsBefore.outEdgeVerticalLevel[parents[0]][i].verticalLevel !=  this.DG.vertLevel.outEdgeVerticalLevel[parents[0]][i].verticalLevel ||
                            (parents.length > 1 && vertLevelsBefore.outEdgeVerticalLevel[parents[1]][i].verticalLevel !=  this.DG.vertLevel.outEdgeVerticalLevel[parents[1]][i].verticalLevel)) {
              result[i] = true;
              continue;
            }
          }
...
Heuristics.prototype = {
  ...
  swapPartnerToBringToSideIfPossible: function ( personId ) {
    ...
    if (partners.length < 2) {
      return;
    }  // single-parent relationship, no partner to swap
  ...
  improvePositioning: function (ranksBefore, rankYBefore) {
    ...
    while (improved && iter < 20) {
      ...
      for (var k = 0; k < orderedRelationships.length; k++) {
        ...
        var leftParent  = (parents.length > 1 && xcoord.xcoord[parents[0]] < xcoord.xcoord[parents[1]]) ? parents[0] : (parents.length > 1 ? parents[1] : parents[0]);
        var rightParent = (parents.length > 1 && xcoord.xcoord[parents[0]] < xcoord.xcoord[parents[1]]) ? parents[1] : parents[0];
    ...
    while (improved && iter < 20) {
      ...
      for (var k = 0; k < orderedRelationships.length; k++) {
        ...
        // only applies to relationships with two partners
        if (parents.length < 2) {
          continue;
        }
