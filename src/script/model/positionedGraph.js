  ...
  findAllRootlessPartners: function (leafAndRootlessInfo) {
    ...
    for (var i = 0; i < leafAndRootlessInfo.parentlessNodes.length; i++) {
      ...
      if ( this.GG.getOutEdges(v).length == 1 && this.GG.getOutEdges(v).length > 0) {
        ...
        if (parents.length < 2) {
          continue;
        }  // single-parent relationship, no partner to pair with
  ...
  computePossibleParentlessNodePermutations: function(maxInitOrderingBuckets, leafAndRootlessInfo, rootlessPartners) {
    ...
    for (var i = 0; i < leafAndRootlessInfo.parentlessNodes.length; i++) {
      ...
      if ( this.GG.getOutEdges(v).length == 1 ) {
        ...
        if (parents.length < 2) {
          // single-parent relationship, no partner to group with
        } else {
  ...
  reRankRelationships: function() {
    ...
    for (var r = 2; r <= this.maxRank; r+=3) {
      ...
      for (var oo = 0; oo < initialOrdering.order[r].length; oo++) {
        ...
        // note: each "relationship" node has one or two "parent" nodes (validate() checks that)

        if (parents.length < 2) {
          // single-parent relationship - place it at the parent's position
          this.moveVertexToRankAndOrder( i, this.ranks[parents[0]], this.order.vOrder[parents[0]] + 1 );
          handled[i] = true;
          continue;
        }
      ...
      for (var oo = 0; oo < initialOrdering.order[r].length; oo++) {
        ...
        if (parents.length < 2) {
          continue;
        }  // single-parent relationships are handled in pass1
            ...
            for (var k = 0; k < edges.length; k++) {
              ...
              if (minLevel >= 2 || (minLevel == 1 && otherVirtualEdge) ) {
                ...
                if (dest == u) {                      // same as !this.GG.isVirtual(u)
                  ...
                  var otherParent = parents.length > 1 ? ((parents[0] == v) ? parents[1] : parents[0]) : null;
                  if (otherParent !== null && this.GG.isVirtual(otherParent)) {       // can only be if the edge goes upwards since relationship nodes
          ...
          if (level > 1) {
            ...
            for (var e = 0; e < edgeInfo[i].over.length; e++) {
              ...
              if ( verticalLevels.outEdgeVerticalLevel[ parents[0] ][ w ].verticalLevel <= level ||
                                 (parents.length > 1 && verticalLevels.outEdgeVerticalLevel[ parents[1] ][ w ].verticalLevel <= level) ) {
                lowerLevel = false;
                break;
              }
            }
